...
function generateAdvancedRustCode(data) {
    ...
    // Check for Hello World template
    if (data.helloWorldMessage) {
        const message = data.helloWorldMessage || 'Hello, Stellar!';
        const code = `// Generated by Tralalero Contracts - Hello World
#![no_std]
...
impl HelloContract {
    ...
    // Check for Counter template
    if (data.counterIncrement !== null && data.counterIncrement !== undefined) {
        const increment = data.counterIncrement || 1;
        const code = `// Generated by Tralalero Contracts - Counter
#![no_std]
...
impl CounterContract {
    ...
    // Generate a simple, valid Soroban contract
    const tokenName = (data.name || 'MyToken').replace(/[^A-Za-z0-9]/g, '');
    const tokenSymbol = (data.symbol || 'TOKEN').toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, 12);
    const decimals = data.decimals || 7;
    const initialSupply = data.supply || 1000000;

    const code = `// Smart Contract: ${tokenName}
// Generated by Tralalero Contracts
#![no_std]
...
#[contract]
pub struct ${tokenName}Contract;

#[contractimpl]
impl ${tokenName}Contract {
    ...
    pub fn initialize(env: Env, admin: Address) {
        ...
        env.storage().instance().set(&SUPPLY_KEY, &${initialSupply}i128);
    }

    /// Get the token name
    pub fn name(_env: Env) -> Symbol {
        symbol_short!("${tokenSymbol}")
    }
    ...
    pub fn total_supply(env: Env) -> i128 {
        env.storage().instance().get(&SUPPLY_KEY).unwrap_or(0i128)
    }
...
// Features: ${features.join(', ') || 'Basic Token'}`;
    ...
    while (currentBlock) {
        switch (currentBlock.type) {
            ...
            case 'hello_world_function':
                ...
                data.name = data.name || 'HelloWorld';
                break;
...
function generateRustCode() {
    ...
    while (currentBlock) {
        ...
        switch (currentBlock.type) {
            ...
            case 'hello_world_function':
                data.helloWorldMessage = currentBlock.getFieldValue('MESSAGE') || 'Hello, Stellar!';
                console.log(`     Hello World: ${data.helloWorldMessage}`);
                break;
            case 'counter_function':
                data.counterIncrement = currentBlock.getFieldValue('INCREMENT') || 1;
                console.log(`     Counter increment: ${data.counterIncrement}`);
                break;
        }
        currentBlock = currentBlock.getNextBlock();
    }
...
function validateBlocks() {
    ...
    // Skip admin validation for simple templates
    ...
    if (data.state && data.state.some(v => !v.name || v.name.trim() === '')) {
...
function createDefaultBlocks() {
    ...
    // Get selected template from appState
    const selectedTemplate = appState.selectedTemplate || 'basic';
    console.log(`🔄 Creando bloques por defecto para template: ${selectedTemplate}...`);
    ...
    try {
        ...
        // Crear bloques según el template seleccionado
        const blocks = [];

        if (selectedTemplate === 'basic') {
            // Hello World template - solo un bloque simple
            const helloBlock = window.blocklyWorkspace.newBlock('hello_world_function');
            if (helloBlock) {
                helloBlock.initSvg();
                helloBlock.render();
                helloBlock.setFieldValue('Hello, Stellar!', 'MESSAGE');
                blocks.push(helloBlock);
                console.log('✅ Bloque Hello World creado');
            }
        } else if (selectedTemplate === 'defi') {
            // Counter template - solo un bloque simple
            const counterBlock = window.blocklyWorkspace.newBlock('counter_function');
            if (counterBlock) {
                counterBlock.initSvg();
                counterBlock.render();
                counterBlock.setFieldValue(1, 'INCREMENT');
                blocks.push(counterBlock);
                console.log('✅ Bloque Counter creado');
            }
        } else if (selectedTemplate === 'rwa') {
            // RWA template - bloques de RWA
            const rwaAsset = window.blocklyWorkspace.newBlock('rwa_asset');
            if (rwaAsset) {
                rwaAsset.initSvg();
                rwaAsset.render();
                blocks.push(rwaAsset);
                console.log('✅ Bloque RWA Asset creado');
            }

            const rwaCustody = window.blocklyWorkspace.newBlock('rwa_custody');
            if (rwaCustody) {
                rwaCustody.initSvg();
                rwaCustody.render();
                blocks.push(rwaCustody);
                console.log('✅ Bloque RWA Custody creado');
            }

            const rwaSettlement = window.blocklyWorkspace.newBlock('rwa_settlement');
            if (rwaSettlement) {
                rwaSettlement.initSvg();
                rwaSettlement.render();
                blocks.push(rwaSettlement);
                console.log('✅ Bloque RWA Settlement creado');
            }

            const rwaCompliance = window.blocklyWorkspace.newBlock('rwa_compliance');
            if (rwaCompliance) {
                rwaCompliance.initSvg();
                rwaCompliance.render();
                blocks.push(rwaCompliance);
                console.log('✅ Bloque RWA Compliance creado');
            }

            const rwaRedemption = window.blocklyWorkspace.newBlock('rwa_redemption');
            if (rwaRedemption) {
                rwaRedemption.initSvg();
                rwaRedemption.render();
                blocks.push(rwaRedemption);
                console.log('✅ Bloque RWA Redemption creado');
            }
        }
